<script lang="ts">
  ...
  const floorTexPaths: Record<string, string> = {
    'light-oak': images.lightOak, 'walnut': images.walnut,
    'bamboo': images.bamboo, 'laminate': images.laminate,
    'ceramic-white': images.ceramicWhite, 'ceramic-gray': images.ceramicGray,
    'porcelain': images.porcelain,
    'marble-white': images.marbleWhite, 'marble-dark': images.marbleDark,
    'carpet-beige': images.carpetBeige, 'carpet-gray': images.carpetGray,
    'concrete': images.concrete, 'slate': images.slate,
    'vinyl': images.vinyl,
  };
  const textureGroups = [
          ...
          <div class="space-y-2">
            ...
            <div class="grid grid-cols-3 gap-1.5">
              ...
              {#each wallColors.filter(wc => wc.texture) as wc}
                {@const texPath = ({ 'red-brick': images.redBrick, 'exposed-brick': images.exposedBrick, 'stone': images.stone, 'wood-panel': images.woodPanel, 'concrete-block': images.concrete, 'subway-tile': images.subwayTile })[wc.id] ?? ''}
                <button
        ...
        {:else}
          ...
          <div class="space-y-2">
            ...
            <div class="grid grid-cols-3 gap-1.5">
              ...
              >None</button>
              {#each wallColors.filter(wc => wc.texture) as wc}
                {@const texPath = ({ 'red-brick': images.redBrick, 'exposed-brick': images.exposedBrick, 'stone': images.stone, 'wood-panel': images.woodPanel, 'concrete-block': images.woodPanel, 'subway-tile': images.subwayTile })[wc.id] ?? ''}
                <button
                  class="rounded-md border-2 text-[10px] text-center h-14 flex flex-col items-center justify-end overflow-hidden relative {sideTex === wc.id ? 'border-blue-500 ring-1 ring-blue-200' : 'border-gray-200 hover:border-gray-300'}"
                  style={texPath ? `background-image: url(${texPath}); background-size: cover; background-position: center;` : `background-color: ${wc.color}20`}
