<script lang="ts">
  ...
  import {placePreset, roomPresets} from '$lib/utils/roomPresets';
  import {setTextureLoadCallback} from '$lib/utils/textureGenerator';
  import type {ProjectSettings} from '$lib/stores/settings';
  import {formatArea, formatLength, projectSettings} from '$lib/stores/settings';
  import type {CanvasState} from '$lib/utils/canvasInteraction';
  import {
    drawAnnotation as _drawAnnotation,
    drawAnnotations as _drawAnnotations,
    drawColumn as _drawColumn,
    drawDoorDistanceDimensions as _drawDoorDistanceDimensions,
    drawDoorOnWall as _drawDoorOnWall,
    drawFurnitureItem,
    drawGuides as _drawGuides,
    drawMinimap as _drawMinimap,
    drawPersistedMeasurements as _drawPersistedMeasurements,
    drawRooms as _drawRooms,
    drawSnapPoints as _drawSnapPoints,
    drawStair as _drawStair,
    drawTextAnnotations as _drawTextAnnotations,
    drawWall as _drawWall,
    drawWallJoints as _drawWallJoints,
    drawWindowDistanceDimensions as _drawWindowDistanceDimensions,
    drawWindowOnWall as _drawWindowOnWall
  } from '$lib/utils/canvasRenderer';
  import {
    findColumnAt as _findColumnAt,
    findDoorAt as _findDoorAt,
    findFurnitureAt as _findFurnitureAt,
    findHandleAt as _findHandleAt,
    findRoomAt as _findRoomAt,
    findStairAt as _findStairAt,
    findWallAt as _findWallAt,
    findWindowAt as _findWindowAt,
    hitTestAnnotation as _hitTestAnnotation,
    hitTestMeasurement as _hitTestMeasurement,
    hitTestTextAnnotation as _hitTestTextAnnotation,
    pointInPolygon,
    positionOnWall
    ...
    for (let c = 0; c < clusterCount; c++) {
      ...
      for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
          ...
          const level = Math.floor(value * 4); // scale về 0-3
  ...
  $effect(() => {
    setInterval(() => {
      ...
      const updatedFloor = {...currentFloor, furniture: currentFloor.furniture.map(item => {
          if (item.catalogId === 'camera') {
            return ({...item, heatmapMatrix});
            // return ({...item, rotation: item.rotation - Math.ceil((Math.random()) * 10 )});
          }
          return item;
        })}
      currentFloor = updatedFloor as any;
